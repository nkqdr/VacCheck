...
# main parts from https://github.com/panzi/verify-ehc   (signature download, qr bytecode interpretation)
# for information on Digital Vaccination Record https://www.q-perior.com/fokusthema/digitaler-impfnachweis-technische-grundlagen-der-eu-und-die-ausgestaltung-der-laender/

# usage example python vaccheck --camera 0    (reads the first camera, and checks qr codes continously)
# more command line arguments see main_prepare()

# program run:
#   main_prepare()     // loads signatures and checks them from servers
#   main()             // main loop for continous camera scan 


# TODO
# MIT-Lizenz einfuegen
# missing: reporting statistics (how many checks, how many invalid codes, how many correct vaccination codes ....)
# Weakness: fixed signature services and its interpretation (better: central server where sigfile is retrieved from)
...
from dateutil.parser import isoparse as parse_datetime                    # critical library  Wirklich benoetigt???
...
def load_hack_certs_json(data: bytes, source: str) -> CertList:
    ...
    for key_id_hex, cert_dict in certs_dict['trustList'].items():
        print(f"not valid before " , cert_dict['notValidBefore'])           # critical  ab hier wegen parse_datetime
...
def decode_ehc(b45_data: str) :  # -> CoseMessage
    ...
    try:
        ...
        print(b45_data)
        print(f'Invalid base45 string. Try with single quotes.')
        valid_ehc = False
        return "",False
...
def verify_ehc(msg: CoseMessage, issued_at: datetime, certs: CertList, print_exts: bool = False) :
    global oldsignature
    ...
    if isinstance(msg, Sign1Message):
        ...
        if (oldsignature == b64encode(msg.signature).decode("ASCII")) : return True, False
        oldsignature = b64encode(msg.signature).decode("ASCII")
    ...
    if not cert:
        print(f'Key ID not found in trust list: {key_id.hex()}')
        return False, False
    ...
    return valid and not cert_expired and not revoked and usage_valid, True
...
# ##############################################################################################################################################################################################
# above is CERT handling only, taken from verify-ehc


# source from here is taken from different sources, mixed and changed

def read_barcodes(frame, certs):
    ...
    for barcode in barcodes:
        ...
        time.sleep(1)
        ehc_codes.append(barcode_info)
        ehc_check(ehc_codes, certs)
...
# added camera support
    ...
    ap.add_argument('--camera', action='store_true', default=False, help='ehc_code must be shown to camera.')
    ...
    return ap, certs, args;

def main(ap, certs,args):
    print("main")
    args = ap.parse_args()
    ehc_codes: List[str] = []
    if args.image:
    ...
    if args.camera:  # this is special: it is a loop for multiple codes
        # camera = cv2.VideoCapture(1)
        print("Open Camera Number " + sys.argv[2])
        camera = cv2.VideoCapture(int(sys.argv[2]),cv2.CAP_DSHOW)
        ret, frame = camera.read()
        engine.say("System bereit")
        engine.runAndWait()
        while ret:                          # main loop ##################################################################################################
            ret, frame = camera.read()
            frame = read_barcodes(frame, certs)
            cv2.imshow('Barcode/QR code reader', frame)
            if cv2.waitKey(1) & 0xFF == 27:                 # press ESC to end
                break    
        camera.release()
...
def ehc_check(ehc_codes, certs):
    for ehc_code in ehc_codes:
        ehc_msg, valid_ehc = decode_ehc(ehc_code)
        if valid_ehc:                  
            ...
            if certs is not None:
                is_ehc, is_ndup = verify_ehc(ehc_msg, issued_at, certs, args.print_exts)
                if (is_ehc and is_ndup): 
                    ...
                    if ("r" in ehc) :
                        ...
                        if ((ehc["v"][0]["dn"]) == (ehc["v"][0]["sd"])) :   # check if total number of doses are reached
                            engine.say("ok")
                            engine.runAndWait()
            ...
            print("decode_ehc wrong")
            wrong_code()

if __name__ == '__main__':

    ap, certs, args = main_prepare();   # this reads the signatures at start
    main(ap, certs, args);              # main loop
